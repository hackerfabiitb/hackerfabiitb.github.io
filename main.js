...
addHover('a, button, .nav-logo, .member-card, .pillar, .tbl-row, .growth-row, .tl-item, .req-row, .s-link, .copyable, .hstat');
  ...
  el.addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      ...
      if (hint) {
        ...
        setTimeout(() => { hint.textContent = 'copy'; hint.classList.remove('copied'); }, 2000);
      }
...
function spawnTerminal() {
  ...
  printLines([{ cls: 'ok', text: 'HackerFab Terminal v1.0' }, { cls: 'out', text: 'Type "help" to get started.' }]);
  input.focus();
